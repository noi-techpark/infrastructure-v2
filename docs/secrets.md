...
```sh
  ...
  EXTRACTED_JSON=`terraform -chdir=infrastructure/terraform/postgres_timeseries output -json | jq -r '{hostname: .postgres_hostname.value, port: 5432, pw_readwrite: .postgres_password_bdp.value, pw_readonly: .postgres_password_bdp_readonly.value}'`
