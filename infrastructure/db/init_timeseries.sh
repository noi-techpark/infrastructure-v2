...
EXTRACTED_JSON=`terraform -chdir=../terraform/db output -json | jq -r '{hostname: .odh_postgres_hostname.value, port: .odh_postgres_port.value, usr_root: .odh_postgres_username.value, pw_root: .odh_postgres_password.value, pw_readwrite: .odh_postgres_password_bdp.value, pw_readonly: .odh_postgres_password_bdp_readonly.value, sg_homeoffice_id: .postgres_homeoffice_sg_id.value}'`
...
SECURITY_GROUP=`jq '.sg_homeoffice_id' -r <<< "$EXTRACTED_JSON"`
# Retrieve current IP address
IP=`curl -s http://checkip.amazonaws.com`
aws ec2 authorize-security-group-ingress --group-id "$SECURITY_GROUP" --protocol tcp --port 5432  --cidr $IP/32 --profile dev --output text --tag-specifications 'ResourceType=security-group-rule,Tags=[{Key=Name,Value=Clemens}]'
